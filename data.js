(() => {
  ...
  window.SCOREBOARD_DATA = {
    players: knockoutPlayers.map(player => {
      const group = groupByName.get(player.name);
      return {
        name: player.name,
        groupPoints: group.groupPoints,
        groupMatches: group.groupMatches,
        knockoutPoints: player.total,
        knockoutRight: player.right,
        total: group.groupPoints + player.total,
      };
    }),
  };
})();
